...
pipeline{
    ...
    stages {
        ...
        stage('Checkout from Git'){
            steps{
                git branch: 'main', url: 'https://github.com/tisanbako/DevSecOps-Project.git'
            }
        }
            ...
            steps{
                script{
                   withDockerRegistry(credentialsId: 'DockerHub-Credential', toolName: 'Docker'){
                       ...
                       sh "docker tag reddit tisanbako/reddit:latest "
                       sh "docker push tisanbako/reddit:latest "
                    }
                }
            }
        }
        stage("Trivy App Image Scan"){
            steps{
                sh "trivy image tisanbako/reddit:latest > trivy_image_analysis_report.txt"
            }
        }
        ...
        stage('ZAP Dynamic Testing | DAST') {
            steps {
                sshagent(['OWASP-Zap-Credential']) {
                    sh 'ssh -o StrictHostKeyChecking=no ubuntu@3.142.199.218 "docker run -t owasp/zap2docker-stable zap-baseline.py -t http://3.18.113.147:30000/" || true'
                                                        //JENKINS_PUBLIC_IP                                                      //ANY_OF_THE_EKS_WORKER_NODE_IP_ADDRESS:3000
                }
            }
        }
    ...
    always {
        ...
        slackSend channel: '#bako-devsecops-cicd-alerts', //update and provide your channel name
